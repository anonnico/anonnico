# Hi there, I'm Niko

🎓 Computer Science student at Munich University of Applied Sciences  
🐧 Linux enthusiast & low-level tinkerer  
💡 Passionate about clean software design, automation, and AI  
🚀 Exploring everything from RISC-V to Docker and cloud infrastructure

---

## 🧠 Current Projects
... 
---

## 🛠 Tech Stack & Tools

> Languages  
- Java, Python, C, Lua, Bash  

> Tools & Frameworks  
- Docker, Git, IntelliJ, VS Code, SPSS, Arch Linux, Hyprland  

> Interests  
- Operating Systems & Low-Level Development  
- DevOps & Infrastructure as Code  
- Machine Learning & AI  
- Biohacking & Cognitive Performance  

---

## 📌 Goals  
- 🎯 Build and publish open-source projects  
- 📚 Earn AWS & Cisco certifications  
- 🤖 Deepen understanding of AI (ML workflows, LLMs, edge AI, etc.)  
- 🧼 Write clean, testable and maintainable code  

---

## 📫 Reach Me  
- 📬 Email: niko.gebrande@icloud.com
- 🔗 LinkedIn: 

---
<!---
anonnico/anonnico is a ✨ special ✨ repository because its `README.md` (this file) appears on your GitHub profile.
You can click the Preview link to take a look at your changes.
--->
